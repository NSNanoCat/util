...
export async function fetch<T>(resource: string | FetchOptions, options: FetchOptions = {
	timeout: 5
	...
	if ($app === "Node.js") {
		...
		const fetchCookie = (globalThis as any).fetchCookie ? (globalThis as any).fetchCookie : await import("fetch-cookie").then(module => module.default);
			...
			await fetch(url, options)
				.then(async (response: any) => {
					...
					return {
						...
						bodyBytes: bodyBytes,
						headers: Object.fromEntries(Object.entries(headers).map(([key, value]) => [key, key.toLowerCase() !== "set-cookie" ? (value as any).toString() : value])),
					};
				})
