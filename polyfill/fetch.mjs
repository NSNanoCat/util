...
export async function fetch(resource, options = {}) {
  resource = mergeConfig(resource, options);
  // 定义请求方法（小写）
  ...
  switch ($app) {
    ...
    default:
      ...
      if (typeof resource.redirection === "boolean")
        resource["auto-redirect"] = resource.redirection;
      // 转换请求体
      ...
      // 判断是否请求二进制响应体
      switch (
        (resource.headers?.Accept || resource.headers?.accept)?.split(";")?.[0]
      ) {
    ...
    case "Quantumult X":
      ...
      if (typeof resource["auto-redirect"] === "boolean")
        _.set(resource, "opts.redirection", resource["auto-redirect"]);
      // 转换请求体
      if (resource.body instanceof ArrayBuffer) {
        ...
        resource.bodyBytes = resource.body.buffer.slice(
          resource.body.byteOffset,
          resource.body.byteLength + resource.body.byteOffset
        );
        resource.body = undefined;
    ...
    case "Node.js": {
      const nodeFetch = globalThis.fetch
        ? globalThis.fetch
        : require("node-fetch");
      const fetchCookie = globalThis.fetchCookie
        ? globalThis.fetchCookie
        : require("fetch-cookie").default;
...
// https://github.com/blackmatrix7 - MagicJS 3
/**
 * 将请求参数转换为查询字符串
 *
 * @param {object} config
...
function paramsToQueryString(config) {
  ...
  if (!!_config.params) {
    ...
    Console.debug(`Params to QueryString: ${_config.url}`);
  }
  return _config;
}
/**
 * 合并参数
 *
 * @param {object|string} resource 请求参数/请求地址
 * @param {object} [options] 请求参数
 * @returns {object}
 */
function mergeConfig(resource, options = {}) {
  // 初始化参数
  switch (typeof resource) {
    ...
    default:
      throw new TypeError(
        `${Function.name}: 参数类型错误, resource 必须为对象或字符串`
      );
  }
  // 自动判断请求方法
  if (!resource.method) {
  ...
  if (resource.timeout) {
    ...
    if (resource.timeout > 500)
      resource.timeout = Math.round(resource.timeout / 1000);
  }
  return paramsToQueryString(resource);
}
