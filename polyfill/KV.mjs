...
export class KV {
     ...
     */
    static #restAPIBaseURL = "https://api.cloudflare.com/client/v4";
    /**
     ...
     */
    static #hasRESTConfigKeys(value) {
     ...
     */
    static #normalizeRestConfig(config) {
     ...
     */
    async #getRESTValue(keyName) {
        const { response, body } = await this.#request("GET", `/values/${encodeURIComponent(keyName)}`, {
            responseType: "text",
     ...
     */
    async #setRESTValue(keyName, value) {
        await this.#request("PUT", `/values/${encodeURIComponent(keyName)}`, {
            body: value,
            headers: {
     ...
     * @returns {Promise<void>}
     */
    async #deleteRESTValue(keyName) {
        await this.#request("DELETE", `/values/${encodeURIComponent(keyName)}`);
    }
    /**
     * 列出 REST 后端中的键。
    ...
    async #request(method, pathname, options = {}) {
        if (typeof globalThis.fetch !== "function")
            throw new TypeError("KV REST API requires global fetch in Node.js.");
        ...
        switch (options.responseType) {
            case "text":
                if (response.ok || options.allowedStatusCodes?.includes(response.status))
                    return { response, body: bodyText };
                throw KV.#createRESTError(response, KV.#parseJSON(bodyText));
            default: {
                const body = bodyText ? KV.#parseJSON(bodyText) : null;
                const isAllowedStatus = options.allowedStatusCodes?.includes(response.status) ?? false;
                if ((response.ok || isAllowedStatus) && body?.success !== false)
                    return { response, body };
                throw KV.#createRESTError(response, body);
            }
        }
    }
    /**
     ...
     * @returns {URL}
     */
    #createRESTURL(pathname, query = {}) {
        const url = new URL(`${KV.#restAPIBaseURL}/accounts/${encodeURIComponent(this.#restConfig.accountId ?? "")}/storage/kv/namespaces/${encodeURIComponent(this.#restConfig.namespaceId ?? "")}${pathname}`);
     ...
     * @returns {Error}
     */
    static #createRESTError(response, body) {
